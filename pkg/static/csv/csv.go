...
// Processor handles CSV file processing
type Processor struct{}

// NewProcessor creates a new CSV processor
func NewProcessor() *Processor {
	return &Processor{}
}

// GetDocuments returns all CSV files as documents
func (p *Processor) GetDocuments(ctx context.Context) ([]types.Document, error) {
	var documents []types.Document

	err := fs.WalkDir(csvFiles, ".", func(path string, d fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
		...
		content, err := csvFiles.ReadFile(path)
		if err != nil {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to walk CSV files: %w", err)
	}

	return documents, nil
}

// ListFiles returns list of all CSV filenames
...
// processFile converts a CSV file to a document using utils functions
func (p *Processor) processFile(filePath string, content []byte) (*types.Document, error) {
	...
	return &types.Document{
		ID:        fmt.Sprintf("csv_%s_%d", strings.TrimSuffix(filename, ".csv"), time.Now().UnixNano()),
		Type:      "csv",
		Title:     filename,
